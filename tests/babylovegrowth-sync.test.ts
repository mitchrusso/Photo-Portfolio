...
test("BabyLoveGrowth routes require private bearer secrets and never publish provider authorship", () => {
  ...
  assert.match(sync, /summaryMatchesStoredArticle/)
  assert.match(sync, /storedBySourceId/)
  assert.match(sync, /activeSourceIds\.length > 0/)
  assert.match(syncRoute, /isProviderThrottle \? "WARNING" : "CRITICAL"/)
