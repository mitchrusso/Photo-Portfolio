...
test("website builder keeps templates above one unified accordion menu", () => {
  ...
  assert.match(source, /Upload logo/)
  assert.match(source, /Show name and logo/)
  assert.match(source, /data-testid="website-live-identity"/)
  assert.match(source, /siteLogoUrl: uploadedPhoto\.url/)
  assert.match(source, /title: "Website logo"/)
  ...
  assert.match(source, /aria-label="Close Template controls"/)
  ...
  assert.match(visitorPreviewSource, /settings\.showSiteIdentity/)
  assert.match(visitorPreviewSource, /settings\.siteLogoUrl/)
  assert.match(visitorPreviewSource, /settings\.siteName\.trim\(\)/)
  assert.match(visitorPreviewSource, /Created with PhotoView\.io/)
...
test("subscriber shortcuts expose referrals and the compact website toolbar", () => {
  ...
  assert.match(toolbarSource, /flex-wrap items-center gap-2 overflow-visible rounded-md border/)
...
test("subscriber dashboard header keeps its title visible while tools scroll only on narrow screens", () => {
  ...
  assert.match(dashboardSource, /context=\{activePanel === "settings" \? "settings" : activePanel === "website" \? "website" : "dashboard"\}/)
  assert.match(dashboardSource, /hideTrigger/)
  assert.match(dashboardSource, /openRequest=\{tourOpenRequest\}/)
...
test("new subscriber workspaces receive one dismissible welcome that launches the persistent Start Here tour", () => {
  ...
  assert.match(schemaSource, /welcomeTourPending\s+Boolean\s+@default\(false\)/)
  assert.match(onboardingSource, /welcomeTourPending: true/)
  assert.match(dashboardPageSource, /isSubscriberWelcomeTourPending/)
  assert.match(dashboardPageSource, /initialWelcomeTourPending=\{welcomeTourPending \|\| \(process\.env\.NODE_ENV !== "production"/)
  assert.match(welcomeRouteSource, /data: \{ welcomeTourPending: false \}/)
  assert.match(dashboardSource, /Welcome to PhotoView\.io/)
  assert.match(dashboardSource, /It’s great to have you here\./)
  assert.match(dashboardSource, /Start Here Tour/)
  assert.match(dashboardSource, /Explore on my own/)
  assert.match(dashboardSource, /openTours\("start-here"\)/)
})
...
test("website help and tooltips describe the unified builder interface", () => {
  ...
  assert.match(helpSource, /left Build your site menu starts with Website identity and Template controls/)
  assert.match(helpSource, /upload an optional PNG, JPG, WebP, or AVIF logo/)
...
test("custom website domains normalize safely and exclude platform or development hosts", () => {
  assert.equal(normalizeCustomDomain("Example.COM"), "example.com")
  assert.equal(normalizeCustomDomain("https://www.example.com"), "www.example.com")
  assert.equal(normalizeCustomDomain("example.com/portfolio"), "")
  assert.equal(normalizeCustomDomain("example.com:8443"), "")
  assert.equal(normalizeCustomDomain("photoview.io"), "")
  assert.equal(normalizeCustomDomain("client.photoview.io"), "")
  assert.equal(normalizeCustomDomain("photo-portfolio.vercel.app"), "")
  assert.equal(normalizeCustomDomain("localhost"), "")
...
test("reference-inspired website elements are selectable, documented, and rendered responsively", () => {
  ...
  assert.match(experienceSource, /Math\.max\(0\.143, requestedHeadlineScale \/ 1\.4\)/)
  ...
  assert.doesNotMatch(experienceSource, />Featured projects</)
  assert.match(experienceSource, /function FullFrameStoryCover/)
  assert.match(experienceSource, /className="h-auto w-full"/)
  assert.match(experienceSource, /aspect-\[4\/5\] w-full max-w-md bg-transparent/)
  assert.doesNotMatch(experienceSource, /max-w-md bg-\[#ded8cb\]/)
  assert.match(experienceSource, /<div className="min-h-screen" data-inspired-template="commercial-casebook">/)
  assert.match(experienceSource, /compact \? "" : "md:grid-cols-3"/)
  assert.match(experienceSource, /flex flex-col items-start gap-3 border-b px-5 py-4 \$\{compact \? "" : "sm:flex-row sm:items-center sm:justify-between sm:gap-5"\}/)
  assert.match(experienceSource, /min-w-0 flex-1 whitespace-normal break-words text-xs font-semibold uppercase leading-5/)
  assert.match(experienceSource, /compact \? "whitespace-normal break-words leading-4" : "truncate"/)
  ...
  assert.match(experienceSource, /w-full min-w-0 max-w-full font-serif[\s\S]*?\[overflow-wrap:anywhere\]/)
  assert.match(experienceSource, /w-full min-w-0 max-w-full font-light[\s\S]*?\[overflow-wrap:anywhere\]/)
  ...
  assert.match(headlineControlsSource, /aria-label="Kinetic headline scroll speed"[\s\S]*?onInput=\{\(event\) => \{[\s\S]*?onSetHeroScrollSpeed/)
  assert.match(headlineControlsSource, /aria-label="Kinetic headline center slowdown"/)
  assert.match(headlineControlsSource, /aria-label="Kinetic headline center slowdown"[\s\S]*?onInput=\{\(event\) => \{[\s\S]*?onSetHeroScrollSlowdown/)
  ...
  assert.match(experienceSource, /compact \? "max-w-\[48vw\] gap-3" : "gap-5"/)
  assert.match(experienceSource, /stories\.slice\(0, compact \? 2 : 5\)/)
...
test("large website image presentations remain full-frame while thumbnails may crop", () => {
  const dashboardSource = readWebsiteBuilderImplementation()
  const previewSource = readFileSync(join(process.cwd(), "src/components/site/website-draft-preview.tsx"), "utf8")

  for (const source of [dashboardSource, previewSource]) {
    assert.match(source, /workDisplayMode === "slideshow"[\s\S]*?bg-transparent[\s\S]*?object-contain/)
    assert.match(source, /workDisplayMode === "film-strip"[\s\S]*?bg-transparent[\s\S]*?object-contain/)
    assert.match(source, /workDisplayMode === "cover-cards"[\s\S]*?bg-transparent[\s\S]*?object-contain/)
    assert.match(source, /portfolioGridDisplayMode === "slideshow"[\s\S]*?bg-transparent[\s\S]*?object-contain/)
    assert.match(source, /portfolioGridDisplayMode === "film-strip"[\s\S]*?bg-transparent[\s\S]*?object-contain/)
    assert.match(source, /portfolioGridDisplayMode === "cover-cards"[\s\S]*?bg-transparent[\s\S]*?object-contain/)
    assert.match(source, /workDisplayMode === "thumbnail-grid"[\s\S]*?object-cover/)
    assert.match(source, /portfolioGridDisplayMode === "thumbnail-grid"[\s\S]*?object-cover/)
  }
})
...
test("website builder page cards expose saved drag ordering and explicit save feedback", () => {
  ...
  assert.match(previewSource, /aria-label="PhotoView\.io policies"/)
  assert.match(previewSource, />Terms<\/a>/)
  assert.match(previewSource, />Privacy<\/a>/)
...
test("Copyright and DMCA policy exposes the designated agent and complete notice process", () => {
  ...
  const responseTemplatesSource = readFileSync(join(process.cwd(), "docs/DMCA-RESPONSE-TEMPLATES.md"), "utf8")
  ...
  assert.match(copyrightSource, /DMCA-1075905/)
  ...
  assert.match(copyrightSource, /Standard technical measures/)
  assert.match(copyrightSource, /https:\/\/www\.copyright\.gov\/dmca-directory\//)
  assert.match(termsSource, /Read the Copyright &amp; DMCA Policy/)
  assert.match(termsSource, /standard technical measures/)
  assert.match(footerSource, /aria-label="PhotoView\.io policies"/)
  assert.match(footerSource, />Copyright &amp; DMCA<\/a>/)
  assert.match(operationsSource, /Registration status: Active as of July 20, 2026/)
  assert.match(operationsSource, /U\.S\. Copyright Office registration number: DMCA-1075905/)
  assert.match(responseTemplatesSource, /Acknowledge receipt/)
  assert.match(responseTemplatesSource, /Forward a counter-notice to the claimant/)
  assert.match(aiHelpSource, /Copyright complaints and DMCA notices/)
  assert.match(aiHelpSource, /Do not use the Bug\/Feature Request form for a legal notice/)
})
...
test("Lightroom guidance and plugin support beginner-friendly new and existing portfolio imports", () => {
  ...
  const initiateRouteSource = readFileSync(join(process.cwd(), "src/app/api/lightroom/import/initiate/route.ts"), "utf8")
  const finalizeRouteSource = readFileSync(join(process.cwd(), "src/app/api/lightroom/import/finalize/route.ts"), "utf8")
  const versionRouteSource = readFileSync(join(process.cwd(), "src/app/api/lightroom/plugin-version/route.ts"), "utf8")
  const versionSource = readFileSync(join(process.cwd(), "src/lib/lightroom-plugin-version.ts"), "utf8")
  ...
  assert.match(dashboardSource, /No Adobe developer account is required\./)
  assert.match(dashboardSource, /“API key” and “private key” mean this same subscriber-specific key/)
  assert.match(dashboardSource, /Private import key copied successfully\./)
  assert.match(dashboardSource, /lightroomCopyStatus === "key-copied" \? "Copied" : "Copy"/)
  assert.match(dashboardSource, /Private import key generated\. Click Copy, then paste it into Lightroom Classic\./)
  assert.match(dashboardSource, /lightroomKeyStatus === "generating"/)
  assert.match(dashboardSource, /placeholder="pvp_imp\.\.\."/)
  assert.match(dashboardSource, /Client ID/)
  assert.match(dashboardSource, /Not required\./)
  assert.match(aiHelpSource, /No Adobe developer account, Adobe API key, Adobe private key, or client ID is required\./)
  ...
  assert.match(dashboardSource, /Each rendered image can be up to 50 MB/)
  assert.match(dashboardSource, /Current release:/)
  assert.match(dashboardSource, /Lightroom retains the saved API URL, private import key, destination, and export settings/)
  assert.match(importRouteSource, /export async function GET/)
  assert.match(initiateRouteSource, /initiateDirectLightroomImport/)
  assert.match(finalizeRouteSource, /finalizeDirectLightroomImport/)
  assert.match(versionRouteSource, /LATEST_LIGHTROOM_PLUGIN_VERSION/)
  assert.match(versionRouteSource, /Cache-Control/)
  assert.match(versionSource, /0\.3\.1\.5/)
  assert.match(importHandlerSource, /LIGHTROOM_IMPORT_MAX_BYTES = 50 \* 1024 \*\* 2/)
  assert.match(importHandlerSource, /createDirectPhotoUpload/)
  assert.match(importHandlerSource, /getPhotoObjectBytes/)
  ...
  assert.match(lightroomPluginSource, /local function decodeJson/)
  assert.match(lightroomPluginSource, /MAX_LIGHTROOM_IMAGE_BYTES = 50 \* 1024 \* 1024/)
  assert.match(lightroomPluginSource, /endpointUrl \.\. "\/initiate"/)
  assert.match(lightroomPluginSource, /endpointUrl \.\. "\/finalize"/)
  assert.match(lightroomPluginSource, /"PUT"/)
  assert.match(lightroomPluginSource, /CURRENT_PLUGIN_VERSION = "0\.3\.1\.5"/)
  assert.match(lightroomPluginSource, /checkForPluginUpdate/)
  assert.match(lightroomPluginSource, /Download update/)
  assert.match(lightroomPluginSource, /openUrlInBrowser/)
  assert.match(lightroomPluginSource, /Updating keeps the saved API URL, private key, destination, and Lightroom export settings/)
  assert.doesNotMatch(lightroomPluginSource, /import "LrJson"/)
  assert.match(lightroomPluginSource, /Client name \(optional\)/)
  assert.match(lightroomPluginSource, /Why files failed:/)
  assert.match(aiHelpSource, /Each rendered Lightroom image can be up to 50 MB/)
})
...
test("homepage and website builder explain flexible storytelling and self-service custom domains", () => {
  const homepageSource = readFileSync(join(process.cwd(), "src/app/page.tsx"), "utf8")
  const addressDialogSource = readFileSync(join(process.cwd(), "src/components/portfolio/website-builder/website-address-dialog.tsx"), "utf8")
  const previewSource = readFileSync(join(process.cwd(), "src/components/site/website-draft-preview.tsx"), "utf8")
  const helpSource = readFileSync(join(process.cwd(), "src/lib/ai-help-knowledge.ts"), "utf8")
  ...
  assert.match(addressDialogSource, /Purchased custom domain/)
...
test("self-service custom domains are workspace-scoped, provider-managed, and routed only after verification", () => {
  const schemaSource = readFileSync(join(process.cwd(), "prisma/schema.prisma"), "utf8")
  const migrationSource = readFileSync(join(process.cwd(), "prisma/migrations/20260804170000_custom_domains/migration.sql"), "utf8")
  const routeSource = readFileSync(join(process.cwd(), "src/app/api/website/domain/route.ts"), "utf8")
  const providerSource = readFileSync(join(process.cwd(), "src/lib/vercel-domains.ts"), "utf8")
  const proxySource = readFileSync(join(process.cwd(), "src/proxy.ts"), "utf8")
  const publicationSource = readFileSync(join(process.cwd(), "src/lib/website-publication.ts"), "utf8")
  const customPageSource = readFileSync(join(process.cwd(), "src/app/site-domain/[domain]/page.tsx"), "utf8")
  const publishRouteSource = readFileSync(join(process.cwd(), "src/app/api/website/publish/route.ts"), "utf8")
  const envSource = readFileSync(join(process.cwd(), ".env.example"), "utf8")

  assert.match(schemaSource, /customDomain\s+String\?\s+@unique/)
  assert.match(schemaSource, /customDomainVerifiedAt\s+DateTime\?/)
  assert.match(migrationSource, /Workspace_customDomain_key/)
  assert.match(routeSource, /getSubscriptionWriteBlock/)
  assert.match(routeSource, /id: \{ not: access\.workspace\.id \}/)
...
test("login request and inbox confirmation use the light PhotoView.io visual system", () => {
  ...
  assert.match(loginSource, /That link signs you in and opens your dashboard automatically/)
  assert.doesNotMatch(loginSource, /Open dashboard after clicking the link/)
  ...
  assert.match(loginFormSource, /Send another login email/)
  assert.match(loginFormSource, /Use only the newest email/)
...
test("dashboard uses persistent Gallery to Portfolio to Photo organization and explains portfolio autosave", () => {
  ...
  const persistenceSource = readFileSync(join(process.cwd(), "src/lib/portfolio-persistence.ts"), "utf8")
  const galleryRouteSource = readFileSync(join(process.cwd(), "src/app/api/portfolio/galleries/route.ts"), "utf8")
  ...
  assert.match(dashboardSource, /Rename “My Gallery” to/)
  assert.match(dashboardSource, /Rename “\$\{group\.name\}” to “\$\{requestedName\}”/)
  assert.match(dashboardSource, /No portfolios or photos will be moved or deleted/)
  ...
  assert.match(dashboardSource, /setPortfolioSaveError\(error instanceof Error \? error\.message/)
  assert.match(dashboardSource, /result\?\.error \|\| "A portfolio change could not be saved\."/)
  assert.match(dashboardSource, /portfolioSaveError \|\| "A change could not be saved\. Please try it again\."/)
  assert.match(persistenceSource, /galleryName: gallery\.galleryName/)
  assert.match(persistenceSource, /class PortfolioGalleryValidationError extends Error/)
  assert.match(persistenceSource, /code: "PASSWORD_REQUIRED"/)
  assert.match(galleryRouteSource, /error instanceof PortfolioGalleryValidationError/)
  assert.match(galleryRouteSource, /status: 400/)
