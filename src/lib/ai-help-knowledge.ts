...
export const aiHelpTopics: AiHelpTopic[] = [
  ...
  },
  {
    ...
    details: [
      ...
      "Open Template controls to adjust colors, upload or replace a custom website background image, use Screen back image to fade it toward the selected background color, and use Brightness to darken or brighten only the background image. You can also choose the font style and set the image frame, line thickness, and image shape. The controls have their own vertical scroll area, and the Live Canvas updates as settings change.",
      ...
      "Choose the overall template from the filmstrip, then open Template controls to set the site background color or upload your own background image, text color, accent color, font style, image frame, line thickness, and image shape. The color remains the fallback when no background image is used.",
