...
export const aiHelpTopics: AiHelpTopic[] = [
  ...
  },
  {
    ...
    details: [
      ...
      "The builder is organized into four simple areas: choose a look from the template film strip, choose a page, edit the live canvas, then adjust style and content in the right panel.",
      "Choose a homepage template from the horizontal template film strip at the top of the builder. The live page preview below updates when a different template is selected, including colors, font style, image frame, image shape, section order, and portfolio display style.",
      ...
      "Click directly into text on the live canvas to edit headlines, intro copy, page copy, trip entries, and contact instructions.",
      "The right panel controls site background color, text color, accent color, font style, image frame, image shape, work source, and how the work is displayed.",
      "To choose the hero image, click the Hero section on the Home page and use the Hero image controls in the right panel. The hero can use the first featured portfolio cover, a specific portfolio cover, a visible photo from the Library, or an uploaded custom image.",
      "Use Homepage section order in the right panel to move Home page sections up or down. The builder canvas updates immediately.",
      "Under Image frame, use the Line thickness slider to control how thick the frame border appears around website images.",
      "For the About page, click the About section and use the right-side panel to upload an optional About photo. If no photo is uploaded, the About page uses a clean text-only layout with no empty image space.",
      ...
      "To set up the contact form destination, open My Website, choose the Contact page, click the contact section, then use the right-side Style and content panel to enter the subscriber-only Form delivery email. That delivery field is not shown to visitors.",
      "The Contact page canvas shows the visitor-facing form only. The public preview menu links are clickable and scroll to the enabled page sections.",
