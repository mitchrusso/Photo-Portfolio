...
export const aiHelpTopics: AiHelpTopic[] = [
  ...
  },
  {
    ...
    details: [
      ...
      "Choose the overall template from the filmstrip, then open Template controls to choose Adaptive Width or Full Screen and set the site background color or upload your own background images, text color, accent color, font style, global site font size, image frame, line thickness, and image shape. The font-size slider works no matter which font you select. PhotoView saves up to 12 recent backgrounds for the website. Select a thumbnail to reuse it, or choose Solid to hide the image without removing it from the saved collection.",
      ...
      "Address lets the subscriber choose a unique name.photoview.io address and connect one purchased custom domain. Enter the preferred visitor address, such as example.com or www.example.com; do not include https:// or a page path. PhotoView also connects its matching root or www address and permanently redirects that alternate address to the preferred one.",
      "After Connect domain, PhotoView looks up the domain's nameservers. When it recognizes Cloudflare, GoDaddy, Squarespace Domains, Namecheap, Wix, Hostinger, IONOS, WordPress.com, or Vercel DNS, Address names that provider and includes a button that opens its DNS management area.",
      "If Set up automatically appears, PhotoView and the detected DNS provider support the free Domain Connect flow for this exact setup. Select it, review the proposed DNS records on the provider's website, and approve them there. PhotoView never asks for the provider password, never changes DNS without approval, and verifies the real DNS result after the provider returns.",
      "If automatic setup is unavailable, use the guided records that remain visible in Address. Open the detected provider, then add the A, CNAME, or TXT records exactly as shown. Each value has a Copy button. Remove only records that conflict with the same host; do not delete unrelated email, MX, SPF, DKIM, or verification records.",
      "Domain Connect is a free industry standard. A provider must approve PhotoView's connection template before Set up automatically can appear. Discovering Domain Connect does not by itself mean one-click setup is ready, so PhotoView keeps guided setup available at all times.",
      ...
      "A provider can report that setup finished before DNS propagation reaches PhotoView. Pending or Needs DNS means wait and select Check connection again; it does not necessarily mean the approved setup failed. DNS changes often appear within minutes but can take up to 48 hours.",
      "The name.photoview.io address remains available during setup and after a custom domain is connected. Removing a custom domain does not unpublish the PhotoView.io website. The subscriber can launch the dedicated Connect my custom domain tour from Tours for the complete automatic and guided workflow.",
