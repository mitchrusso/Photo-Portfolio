...
const walkthroughs: Record<WebsiteWalkthroughGoal, WebsiteWalkthrough> = {
  ...
  "settings-overview": {
    ...
    steps: [
      ...
      { id: "settings-imports", title: "Publish directly from Lightroom", description: "Install the Lightroom Classic plugin, copy the PhotoView.io API URL, and generate the private import key. No Adobe client ID or developer credentials are required. Then create a new portfolio or add selected photographs to an existing one.", destination: { kind: "settings", tab: "imports" } },
