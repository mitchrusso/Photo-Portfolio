...
async function findGallery(tokenValue: string) {
  const target = parseSecureShareToken(tokenValue)
...
export async function GET(request: NextRequest, { params }: RouteProps) {
  ...
  return NextResponse.json({
    unlocked: gallery.privacy !== "PASSWORD" || isOwner || verifyGalleryAccessToken(accessToken, gallery.id),
  })
}

export async function POST(request: NextRequest, { params }: RouteProps) {
  const { token } = await params
  const limit = await checkRequestRateLimit(`secure-gallery-unlock:${token.slice(0, 80)}:${requestClientKey(request)}`, 10, 15 * 60 * 1000)
  if (!limit.allowed) {
    return NextResponse.json(
