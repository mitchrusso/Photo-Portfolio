...
3. **Backups and recovery:** Neon point-in-time recovery, a manual snapshot, and an isolated historical-branch restore drill were confirmed on July 20, 2026. The restored branch matched all production Workspace, Gallery, Photo, and migration counts and was deleted after verification. The active R2 bucket is private and all 1,170 referenced objects passed an existence check. Before expanding beyond the controlled beta, add the separate locked media-backup bucket described in `docs/DISASTER-RECOVERY-RUNBOOK.md`; do not lock the active bucket because subscriber deletions must continue to work.
